...
class  AppFirebaseApi(
    ...
    private val TIEMED_APP_REPOSITORY = "INSPECTION_REPOSITORY"

    /* ********************************* DEVICES ************************************************ */
    suspend fun getDeviceList(): List<Device> {
        var data = emptyList<Device>()
        Log.d(TIEMED_APP_REPOSITORY, "Device list fetching started")
        ...
        if (result.isSuccessful) {
            ...
            Log.d(TIEMED_APP_REPOSITORY, "Device list fetched")

        } else {
            Log.d(TIEMED_APP_REPOSITORY, "Device list fetching error")
        }
        return data
    ...
    suspend fun getDevice(deviceId: String): Device? {
        ...
        Log.d(TIEMED_APP_REPOSITORY, "Device record fetching started")
        val documentReference = firebaseFirestore.collection("devices")
            .document(deviceId)
        val result = documentReference.get()
        result.await()
        if (result.isSuccessful) {
            ...
            Log.d(TIEMED_APP_REPOSITORY, "Device record fetched")

        } else {
            Log.d(TIEMED_APP_REPOSITORY, "Device record fetching error")
        }
        return device
    }
    fun createDevice(device: Device): Flow<Resource<Boolean>> = flow {
        ...
        Log.d(TIEMED_APP_REPOSITORY, "Device record creating started")
        ...
        if (result.isSuccessful) {
            ...
            Log.d(TIEMED_APP_REPOSITORY, "Device record created")

        } else {
            emit(Resource(ResourceState.ERROR, false,null))
            Log.d(TIEMED_APP_REPOSITORY, "Device record creating error")
    ...
    fun  updateDevice(device: Device): Flow<Resource<Boolean>> = flow {
        ...
        Log.d(TIEMED_APP_REPOSITORY, "Device record updating started")
        ...
        if (result.isSuccessful) {
            ...
            Log.d(TIEMED_APP_REPOSITORY, "Device record updated")
        } else {
            emit(Resource(ResourceState.ERROR, false))
            Log.d(TIEMED_APP_REPOSITORY, "Device record updating error")
        }
    ...
    fun uploadSignature(signatureId: String, signatureBytes: ByteArray): Flow<Resource<String>> = flow {
        ...
        Log.d(TIEMED_APP_REPOSITORY, "Signature uploading started")
        ...
        val documentReference = firebaseStorage.getReference("signatures")
            ...
            result.await()
                    if (result.isSuccessful) {
                        ...
                        Log.d(TIEMED_APP_REPOSITORY, "Signature uploaded")

                    } else {
                        emit(Resource(ResourceState.ERROR, null))
                        Log.d(TIEMED_APP_REPOSITORY, "Signature uploading error")
                    }

    }
    fun getSignature(signatureId: String): Flow<Resource<ByteArray>> = flow {
        Log.d(TIEMED_APP_REPOSITORY, "Signature fetching started")
        emit(Resource(ResourceState.LOADING, null))
        val documentReference = firebaseStorage.getReference("signatures")
            .child("${signatureId}.jpg")
            ...
            result.await()
                    if (result.isSuccessful){
                        ...
                        Log.d(TIEMED_APP_REPOSITORY, "Signature fetched")
                    } else {
                        emit(Resource(ResourceState.LOADING, null))
                        Log.d(TIEMED_APP_REPOSITORY, "Signature fetching error")
                    }
    }
    ...
    suspend fun getHospitalList(): List<Hospital> {
        ...
        Log.d(TIEMED_APP_REPOSITORY, "Hospital list fetching started")
        ...
        if(data.isSuccessful) {
            ...
            Log.d(TIEMED_APP_REPOSITORY, "Hospital list fetched")

        } else {
            Log.d(TIEMED_APP_REPOSITORY, "Hospital list fetching error")
        }
        return hospitalList
    }
    ...
    suspend fun getEstStateList(): List<EstState> {
        ...
        Log.d(TIEMED_APP_REPOSITORY, "Est states list fetching started")
        val documentReference = firebaseFirestore.collection("est_states")
        val data = documentReference.get()
        data.await()
        if(data.isSuccessful) {
            hospitalList = data.result.toObjects(EstState::class.java)
            Log.d(TIEMED_APP_REPOSITORY, "Est states list fetched")

        } else {
            Log.d(TIEMED_APP_REPOSITORY, "Est states list fetching error")
        }
        return hospitalList
    }
    ...
    suspend fun getRepairStateList(): List<RepairState> {
        ...
        Log.d(TIEMED_APP_REPOSITORY, "Repair states list fetching started")
        ...
        if(data.isSuccessful) {
            ...
            Log.d(TIEMED_APP_REPOSITORY, "Repair states list fetched")

        } else {
            Log.d(TIEMED_APP_REPOSITORY, "Repair states list fetching error")
        }
    ...
    suspend fun getInspectionStateList(): List<InspectionState> {
        ...
        Log.d(TIEMED_APP_REPOSITORY, "Inspection states list fetching started")
        ...
        if(data.isSuccessful) {
            ...
            Log.d(TIEMED_APP_REPOSITORY, "Inspection states list fetched")

        } else {
            Log.d(TIEMED_APP_REPOSITORY, "Inspection states list fetching error")
        }
