...
class RepairRepositoryFake: RepairRepository {
    ...
    override fun getRepair(repairId: String): Flow<Resource<Repair>> {
        return repairListFlow
            .map {
                val repair: Repair = it.data?.find { it.repairId == repairId } ?: com.example.test.data_generators.repair(
                    "0"
                )
    ...
    override fun insertRepair(repair: Repair): Flow<Resource<String>> {
        ...
        return repairListFlow
            .map {
                val repair: Repair = it.data?.find { it.repairId == repair.repairId } ?: com.example.test.data_generators.repair(
                    "0"
                )
